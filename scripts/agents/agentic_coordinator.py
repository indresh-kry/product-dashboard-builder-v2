...
class AgenticCoordinator:
    ...
    def run_analysis(self, run_hash: str, run_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Initialize chart mapper for this run
        chart_mapper = ChartMapper(run_hash)
        ...
        for agent_type in enabled_agents:
            try:
                ...
                # Get chart information for this agent
                charts_info = chart_mapper.format_charts_for_prompt(agent_type)
                if charts_info:
                    chart_summary = chart_mapper.get_chart_summary(agent_type)
                    print(f"📊 Found {chart_summary['total_charts']} charts for {agent_type} ({chart_summary['primary_charts_count']} primary, {chart_summary['secondary_charts_count']} secondary)", file=sys.stderr)
                
                # Run analysis
                if isinstance(agent, LLMAgent):
                    agent_result = agent.analyze_with_llm(run_hash, run_metadata, charts_info)
                else:
                    agent_result = agent.analyze(run_hash, run_metadata, charts_info)
